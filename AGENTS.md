...
Do not slash-pick skills. Do not skip Design Gate. Copy `skill_md_sha256` and fill `procedure_evidence` from the brief; a boolean is not skill execution. Flagship lookdev needs `evidence/lookdev/` screenshots; a crushed dump versus a lit reference is a fail. Flagship production also needs director/modeler/prop/materials/lookdev receipts — export YAML is not craft. A sphere/cylinder dump or macro lookdev crop fails. Physical products require `/hard-surface`. Industrial-form products also require ACOS-15 spec, ACOS-16 clay, ACOS-17, and Product Form Gate APPROVED before lookdev, production GLB, or web. Brand clone is not the category package — write `reads_as`; clay cannot advance without `product_read_verdict: pass`, `package_fit_ok: true`, `first_dump_verdict: fail`, `clay_iteration >= 2`, and `form_scene.yaml` bounds. Mood/product stills still route ACOS-02. Form critics use a distinct `ACOS_HOST_CONTEXT_ID` and `form-critic-pass`. Do not self-attest critic independence if this chat produced the implementation. A new chat with a distinct `ACOS_HOST_CONTEXT_ID` runs `python tools/host_driver/run_stage.py critic-pass`. `python tools/host_driver/run_stage.py check` is the mechanical audit. A clear premium 3D prompt follows `docs/FLAGSHIP_PREMIUM_WORKFLOW.md` — Blender authors the hero; do not ship a convenience primitive. If Blender MCP/app is down, tell the user, wait, confirm after connect (`confirm-blender --mcp-live`), then start. Never skip.
