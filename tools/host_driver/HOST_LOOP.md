...
- Flagship craft is checked: director / modeler / prop-artist / materials / lookdev receipts. `hard-surface` only for weapons / vehicles / sci-fi. A primitive-named or sphere/plane GLB, an unchecked modeling checkbox, or a macro lookdev crop fails production.
...
- Category package is not a brand clone. Spec must include `reads_as`. Producer cannot write `product_read_verdict: pass` — that is ACOS-17 in a distinct chat. Advance to critic only with `ready_for_critic`, `package_fit_ok: true`, `first_dump_verdict: fail`, `clay_iteration >= 2`, `direction/form_scene.yaml` part bounds, and `direction/clay_look.yaml` matching the current clay PNG hashes. Unread clay stays `fail` in FORM_AUTHORING. Run `tools/form/build_form.py` — no `_clay_iterN.py`.
